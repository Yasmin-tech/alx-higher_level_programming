...
def matrix_mul(m_a, m_b):
    ...
    for row_a in range(len(m_a)):  # rows in m_a
        for col_b in range(len(m_b[0])):  # columns in m_b
            new_elememt = 0
            #  get the element for each row and column; row in m_b
            for k in range(len(m_b)):
                new_elememt += m_a[row_a][k] * m_b[k][col_b]
            # print("({}, {})".format(row_a, col_b))
            res[row_a][col_b] = new_elememt
    return (res)
